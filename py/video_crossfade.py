...
class VideoCrossfadeNode(ComfyNodeABC):
    ...
    async def generate_crossfade(
        ...
        mode_names = {
            ...
            "fade_to_black": "闪黑",
            "fade_to_white": "闪白",
            "fade_to_custom": f"闪自定义色 ({background_color})",
            "additive_dissolve": "叠加",
            "chromatic_dissolve": "色散叠化",
        }
        
        print(f"🎬 开始生成视频转场...")
        print(f"   转场模式: {mode_names.get(transition_mode, transition_mode)}")
        ...
        print(f"🎨 背景颜色: {background_color} -> BGR{bg_color_bgr}")
        ...
        print(f"✅ 叠化转场完成，输出: {video_tensor.shape}")
        return (video_tensor,)
