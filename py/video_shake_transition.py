...
class VideoShakeTransitionNode(ComfyNodeABC):
    ...
    async def generate_shake_transition(
        ...
        print(f"📳 开始生成抖动转场（剪映风格）...")
        print(f"   抖动样式: {shake_style}")
        print(f"   抖动强度: {shake_intensity}")
        print(f"   抖动频率: {shake_frequency}")
        print(f"   转场点: {transition_point}")
        ...
        print(f"✅ 抖动转场完成，输出: {video_tensor.shape}")
    ...
    <script>
        class ShakeController {{
            ...
            init() {{
                ...
                this.ready = true;
                console.log('📳 ShakeController初始化完成, 样式=' + this.shakeStyle);
            }}
