...
class VideoCubeTransitionNode(ComfyNodeABC):
    ...
    async def generate_cube_transition(
        ...
        print(f"🎬 开始生成视频立方体转场（批处理优化）...")
        print(f"   转场方向: {'水平（左右转）' if direction == 'horizontal' else '垂直（上下转）'}")
        ...
        cube_width = width
        cube_height = height
        print(f"📐 立方体面尺寸: {cube_width}x{cube_height}px (匹配画布)")
        ...
        print(f"✅ 立方体转场完成，输出: {video_tensor.shape}")
    ...
    <script>
        class CubeController {{
            constructor() {{
                ...
                console.log('🎬 CubeController初始化完成, 方向=' + this.direction + ', 面尺寸=' + this.cubeWidth + 'x' + this.cubeHeight);
            }}
            ...
            updateRotation(progress) {{
                ...
                console.log(`[JS] 更新: direction=${{this.direction}}, angle=${{angle.toFixed(1)}}°, scale=${{scale.toFixed(2)}}, transform=${{transform}}`);
            }}
    ...
    def _compose_frame_with_opencv(self, frame1_data, frame2_data, cube_transforms, bg_color_bgr, width, height, cube_size):
        ...
        for face_data in faces_to_draw:
            ...
            try:
                ...
                print(f"⚠️  绘制面 {face_data['name']} 时出错: {e}")
                pass
