...
class VideoFlip3DTransitionNode(ComfyNodeABC):
    ...
    async def generate_flip_transition(
        ...
        print(f"🎬 开始生成视频3D翻转转场（批处理优化）...")
        print(f"   转场类型: {transition_type}")
        ...
        print(f"✅ 3D翻转转场完成，输出: {video_tensor.shape}")
    ...
    <script>
        class FlipController {{
            constructor() {{
                ...
                console.log('🎬 FlipController初始化完成, 转场类型=' + this.transitionType);
            }}
            ...
            updateRotation(progress) {{
                ...
                console.log(`[JS] 更新: angle=${{angle.toFixed(1)}}°, type=${{this.transitionType}}`);
            }}
        }}
