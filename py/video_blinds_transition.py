...
class VideoBlindsTransitionNode(ComfyNodeABC):
    ...
    async def generate_blinds_transition(
        ...
        print(f"🎬 开始生成百叶窗转场（批处理优化）...")
        print(f"   百叶窗方向: {blinds_direction}")
        print(f"   百叶窗数量: {blinds_count}")
        ...
        print(f"✅ 百叶窗转场完成，输出: {video_tensor.shape}")
    ...
    <script>
        class BlindsController {{
            ...
            init() {{
                ...
                this.createBlinds();
                this.ready = true;
                console.log('🎬 BlindsController初始化完成, 方向=' + this.direction + ', 数量=' + this.blindsCount);
            }}
